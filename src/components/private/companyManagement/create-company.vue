<template>
  <div class="modal-overlay flex justify-center items-center fixed inset-0 bg-black bg-opacity-50 z-[99999]">
    <div v-if="loading" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100000]">
      ...
      <form @submit.prevent="submitForm" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div v-for="field in formFields" :key="field.id" class="form-group flex flex-col">
          ...
          <div v-else-if="field.type === 'file'" class="flex flex-col w-full">
            <input :id="field.id" type="file" class="hidden" :accept="field.fileTypes || '*'"
            ...
            <label :for="field.id" :class="[
              ...
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                <svg v-if="!loadingFiles[field.model]" class="w-8 h-8 mb-4 text-gray-500" aria-hidden="true"
                ...
                <div v-else class="flex items-center space-x-2">
                  <span
                    class="loader w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></span>
                ...
                <p class="text-xs text-gray-500">
                  {{ uploadedFiles[field.model]?.name ? uploadedFiles[field.model]?.name : field.fileTypes ? `Accepted:
                  ${field.fileTypes}` : "All file types accepted" }}
                </p>
              </div>
            </label>
          </div>
