      ...
      <table class="w-full text-left text-sm text-gray-700">
        ...
        <tbody>
          <tr v-for="customer in filteredCustomers" :key="customer.id" class="border-b hover:bg-gray-50 text-nowrap">
            ...
            <td class="p-4">
              <ul>
                <li v-for="module, index in parseAccessModules(customer.accessModule)" :key="module.name"
                  class="text-gray-700">
                  {{ index + 1 + " - " + module.name }}
                </li>
              </ul>
...
import AddCustomerModal from "./create-subscription.vue";
...
export default {
  ...
  methods: {
    parseAccessModules(accessModule) {
      if (accessModule === null || accessModule === undefined || typeof accessModule !== "string") {
        return [];
      }
      try {
        // Parse the accessModule JSON strings into objects
        const modules = JSON.parse(accessModule);
        return modules.map((module) => JSON.parse(module.value));
      } catch (error) {
        console.error("Error parsing access modules:", error);
        return [];
      }
    },
    async fetchCustomers() {
      try {
        const response = await http.get(
          `/api/subscription-managements?pagination[page]=${this.currentPage}&pagination[pageSize]=${this.pageSize}`,
        );
