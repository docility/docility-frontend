    ...
    <form class="overflow-auto" @submit.prevent="submitRiskCategory">
      ...
      <div v-if="!fileUploaded" class="grid gap-2 grid-cols-1 w-[100%] p-2 items-center justify-center mt-5 bg-primary-text">
        ...
        <div class="mb-4">
          ...
          <textarea v-model="description" rows="5" required class="mt-1 block w-full p-2 border border-gray-300 rounded text-lg"></textarea>
        </div>  
      </div>
      ...
      <div v-if="fileUploaded" class="grid gap-2 grid-cols-1 w-[100%] p-2 items-center justify-center mt-5">
        <div v-for="(item, index) in excelData" :key="index" class="grid gap-2 grid-cols-1 w-[100%] p-2 items-center justify-center mt-5 bg-primary-text">  
          <div class="mb-4">
            <label class="block text-lg font-medium text-gray-700">Risk Type</label>
            <input readonly v-model="item['Risk Type']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
          </div>
          <div class="mb-4">
            <label class="block text-lg font-medium text-gray-700">Risk category/domain</label>
            <input readonly v-model="item['Risk Category']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
          </div>
          <div class="mb-4">
            <label class="block text-lg font-medium text-gray-700">Description of risk category/domain</label>
            <textarea readonly v-model="item['Risk Description']" required class="mt-1 block w-full p-2 border border-gray-300 rounded text-lg text-wrap"></textarea>
          </div> 
        </div>
      </div>
      ...
      if (!this.fileUploaded) {
        ...
        try {
          const response = await http.post('/api/risk-categories', {
            data: { category_domain: this.category, description: this.description, categoryType: this.categoryType },
          });
        ...
        try {
          ...
          for (const item of this.excelData) { 
            categories.push({ category_domain: item['Risk Category'], categoryType: item['Risk Type'],  description: item['Risk Description'] })
          }
          console.log(categories)
          const response = await http.post('/api/create-bulk/risk-categories', 
              categories,
          ); 
