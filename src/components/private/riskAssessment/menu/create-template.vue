  ...
  <div class="container flex flex-col w-full h-full overflow-hidden ">
    ...
    <form class="overflow-auto"  @submit="submitInformationAssetCategory">
      ...
      <div v-if="fileUploaded" class="grid gap-2 grid-cols-1 w-[100%] p-2 items-center justify-center mt-5 bg-primary-text">
        ...
        <div   class="mb-4">
          <label class="block text-lg font-medium text-gray-700">Company</label> 
          <input readonly v-model="item['Companies']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
        </div>

        <div class="mb-4">
          <label class="block text-lg font-medium text-gray-700">Risk treatment option  </label>
          <input readonly v-model="item['Treatment Option']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
        </div>
        <div class="mb-4">
          <label class="block text-lg font-medium text-gray-700">Risk treatment description   </label>
          <input readonly v-model="item['Description']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
        </div>   
        <div class="mb-4">
          <label class="block text-lg font-medium text-gray-700">When To Use   </label>
          <input readonly v-model="item['When To Use']" type="text" required class="mt-1 block w-full p-2 border border-gray-300 rounded" />
        </div>  
        </div>
      </div>
      ...
      async submitInformationAssetCategory(e) { 
        ...
        try {
          ...
          for (const item of this.excelData) { 
            treatment.push({ company: item['Companies'], treatmentOption: item['Treatment Option'],  whenToUse: item['When To Use'], description: item['Description']})
          }
          console.log(treatment)
          const response = await http.post('/api/create-bulk/risk-treatment', 
            treatment,
          ); 
